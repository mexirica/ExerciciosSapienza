...
def ex1(acn1, acn2, acn3, imd_acn1, imd_acn2, init_amount, transact_log):
    acn_balances = {acn1: init_amount, acn2: init_amount, acn3: init_amount}
    imd_balances = {imd_acn1: 0, imd_acn2: 0}
    debt_log = {imd_acn1: {acn1: 0, acn2: 0, acn3: 0}, imd_acn2: {acn1: 0, acn2: 0, acn3: 0}}
    
    for transaction in transact_log:
        (sender_acn, receiver_acn), amount, intermediary_acn, commission_percentage = transaction
        
        # Calcula a comissão da transação
        commission = amount * commission_percentage / 100

        # Verifica se o remetente tem saldo suficiente para efetuar a transação
        if acn_balances[sender_acn] >= amount + commission:
            
            #Efetua a transação
            imd_balances[intermediary_acn] += commission
            acn_balances[sender_acn] -= (amount + commission)
            acn_balances[receiver_acn] += amount
                
        else:
            #Se tiver o dinheiro da comissão
            if acn_balances[sender_acn] > commission:
                acn_balances[sender_acn] -= commission
                imd_balances[intermediary_acn] += commission
            else:#Se não tiver
                if intermediary_acn == imd_acn1:
                    debt_log[imd_acn1][sender_acn] += -(commission - acn_balances[sender_acn])
                    imd_balances[intermediary_acn] += acn_balances[sender_acn]
                    acn_balances[sender_acn] = 0
                else:
                    debt_log[imd_acn2][sender_acn] += -(commission - acn_balances[sender_acn])
                    imd_balances[intermediary_acn] += acn_balances[sender_acn]
                    acn_balances[sender_acn] = 0
        #Se quem recebeu dinheiro deve o ambos intermediarios
        if debt_log[imd_acn1][receiver_acn] < 0 and debt_log[imd_acn1][receiver_acn] == debt_log[imd_acn2][receiver_acn]:
            if acn_balances[receiver_acn] >= (debt_log[imd_acn1][receiver_acn]*2):
                debt_log[imd_acn1][receiver_acn] = 0
                debt_log[imd_acn2][receiver_acn] = 0
                acn_balances[receiver_acn] -= (debt_log[imd_acn1][receiver_acn]*2)
            else:
                debt_log[imd_acn1][receiver_acn] += acn_balances[receiver_acn]/2
                debt_log[imd_acn2][receiver_acn] += acn_balances[receiver_acn]/2
                acn_balances[receiver_acn] = 0
            #Se deve mais o 1 intermediario
        elif debt_log[imd_acn1][receiver_acn] > debt_log[imd_acn2][receiver_acn]:
          if acn_balances[receiver_acn] >= debt_log[imd_acn1][receiver_acn]:
              acn_balances[receiver_acn] -= debt_log[imd_acn1][receiver_acn]
              if acn_balances[receiver_acn] > 0 and debt_log[imd_acn2][receiver_acn] < 0:
                  debt_log[imd_acn2][receiver_acn] += acn_balances[receiver_acn]
                  imd_balances[imd_acn2] += acn_balances[receiver_acn]
                  acn_balances[receiver_acn] = 0
        elif debt_log[imd_acn1][receiver_acn] < debt_log[imd_acn2][receiver_acn]:
          if acn_balances[receiver_acn] >= debt_log[imd_acn2][receiver_acn]:
              acn_balances[receiver_acn] -= debt_log[imd_acn2][receiver_acn]
              if acn_balances[receiver_acn] > 0 and debt_log[imd_acn1][receiver_acn] < 0:
                  debt_log[imd_acn1][receiver_acn] += acn_balances[receiver_acn]
                  imd_balances[imd_acn1] += acn_balances[receiver_acn]

                  acn_balances[receiver_acn] = 0


    # Cria as listas de output
    acn_balances_list = [acn_balances[acn1], acn_balances[acn2], acn_balances[acn3]]
    imd_balances_list = [imd_balances[imd_acn1], imd_balances[imd_acn2]]
    debt_log_list = [[debt_log[imd_acn1][acn1], debt_log[imd_acn1][acn2], debt_log[imd_acn1][acn3]],
                    [debt_log[imd_acn2][acn1], debt_log[imd_acn2][acn2], debt_log[imd_acn2][acn3]]]




    # Retorna a lista de output
    return (acn_balances_list, imd_balances_list, debt_log_list)
